...
exports.updateRole = catchAsyncErrors( async (req, res, next) => {
  ...
  if(req.user.role !== "admin_one"){
    new Errorhandler(`You are not authorize to do this action`)
  }
...
exports.addStrike =catchAsyncErrors(async(req,res,next)=>{
  ...
  if(users.role === "admin_one"){
    return next(new Errorhandler("Admin_one cannot have strikes", 400));
  }
  if(users.strikes.length >=5){
    return next(new Errorhandler("This account cannot have more strikes", 400));
  }
  const strike ={
    subject,
    Description
...
exports.changeBadges =catchAsyncErrors(async(req,res,next)=>{

  if(req.user.role !== "admin_one"){
    return next(new Errorhandler("You are not allowed for this action", 400));
  }
  const newUserData={
    badge : req.query.rank
  }
    // We will add cloudinary later
...
if (!req.query.strid ) {
    return next(new Errorhandler("Please provide the id of the strike", 400));
  }
  ...
  const strikes = user.strikes.filter((s)=>s._id.toString() !== req.query.strid.toString());
