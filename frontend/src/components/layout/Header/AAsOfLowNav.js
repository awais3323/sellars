...
const AAsOfLowNav = () => {
  ...
  return (
    ...
    {
      arrayUniqueByKey && arrayUniqueByKey.map((s)=>(   <span className="Linker" key={s} style={{backgroundColor:`${modes?dark.bg:light.bg}`,border:`${modes?"2px solid rgba(255, 255, 255, 0.593)":"2px solid rgba(128, 128, 128, 0.379)"}`,color:`${modes?"rgba(255, 255, 255, 0.793)":dark.color}`}} onClick={(e)=>searchSubmitHandler(e,s)}>
        {s}
      </span>
      ))
    }
