...
const MainContentBox = React.memo(() => {
  ...
  // console.log(orders?.filter((s)=>s.orderStatus === "delivered"))
  return (
    <>
        ...
        <div className="conBox1">
          ...
          <div className="inConBox1_2 bx">
            <div className="inConBox1_2_1 evheadBox">
              <h4>Undelivered Orders {pendingOrders?.length}</h4>
        ...
        <div className="longBox1">
          <div className="longBox1_1 bx">
            <div className="inLongBox1_1_1 evheadBox">
              <h4>All Orders ( <span className="coloror">Pen : {orders?.filter(s=>s.orderStatus === "pending").length}</span> , <span className="coloror">Pack : {orders?.filter(s=>s.orderStatus === "packed").length}</span> , <span className="coloror">Trans: {orders?.filter(s=>s.orderStatus === "transfered").length}</span>  , <span className="coloror">Deliv: {orders?.filter(s=>s.orderStatus === "delivered").length}</span>, <span className="coloror">Cancel: {orders?.filter(s=>s.orderStatus === "cancelled").length}</span>)</h4>
              <VscRefresh className="refericon" onClick={()=>dispatch(getSellerOrders())}/>
